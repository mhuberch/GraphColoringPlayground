...
const self = {
    ...
    setGraphBackground: (whichone: number): void => {
        if (whichone === 0) {
            ...
            GraphState.repaint();
            window.network.on("beforeDrawing", function(ctx) {
                ctx.drawImage(document.getElementById("bundeslaender"), -250, -230);
            });
            GraphState.repaint();
        }
        else if (whichone === 3) {
            ...
            GraphState.repaint();
            window.network.on("beforeDrawing", function(ctx) {
                ctx.drawImage(document.getElementById("schweiz"), -400, -260);
            });
            GraphState.repaint();
        }
        else if (whichone === 4) {
